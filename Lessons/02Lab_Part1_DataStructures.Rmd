...
# Creates a new vector as a combination of vectors 1 and 4
c(vector1, vector4)

# Creates a vector as a product function of vectors 1 and 4
vector1 * vector4

# Creates a vector with each value 10x vector 1
vector1 * 10

# Turn the characters to uppercase in vector 2
toupper(vector2) 

# Inverts the logical values in vector 3
!vector3
...
diff_from_avg <- temps_F-average_temp
...
matrix1[4, 1] # Returns value in row 4 and column 1 of matrix 1

matrix1[4,  ] # Returns row 4 of matrix 1

matrix1[ , 3] # Returns column 3 of matrix 1 

matrix1[c(12, 14)] # Returns the 12th and 14th element of matrix 1

matrix1[c(12:14)] # Returns the 12th through 13th element of matrix 1

matrix1[2:4, 1:3] # Returns rows 2 through 4 and columns 1 through 3 of matrix 1
...
matrix4 <- matrix(
  ...
  dimnames = list(
    rnames, 
    cnames)
  ) # Constructs a matrix with 2 rows and 3 columns with the values assigned above with row names R1 and R2 and column names C1, C2, and C3

matrix4 # Calls the matrix previous constructed

matrix4['R2','C1'] # Calls the value in R2 and C1 of matrix 4 using dimension names
```

### Exercise 1b: Matrices
Compute the mean of column "C2" in matrix4. (Should be 14.5)
```{r Matrix Exercise}
exercise_1b <- mean(matrix4[,'C2'])
```
...
# Pulls list just created
list1

# Pulls age element of list 1
list1$age

# Provides the titles of each element of list 1
names(list1)

# Pulls first and second elements of list 1
list1[1:2]

# Replaces name element of list 1 with Janelle
list1$name = "Janelle"

# Replaces the fourth element of list 1 with 44
list1[[4]] = 44
```
